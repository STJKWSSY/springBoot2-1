...
1. **小写字母**：前缀应该全部使用小写字母。
2. **连字符分隔**：如果前缀包含多个单词，通常使用连字符（`-`）来分隔单词，而不是下划线（`_`）或其他符号
...
#前缀的规范形式
#小写字母：前缀应该全部使用小写字母。
#连字符分隔：如果前缀包含多个单词，通常使用连字符（-）来分隔单词，而不是下划线（_）或其他符号
my-car.brand= BYD
my-car.price= 10000
```
 ...
 * dog属性配置类
 *
 * @EnableConfigurationProperties  1.开启Car配置绑定功能 2.自动注册到容器中 ,这里就不需要将对象注册到spring容器中 @Component不需要了
 * 先将DogConfig类注册到容器中
 */
@Configuration
...
@Documented
@Inherited
//  @Import(AutoConfigurationPackages.Registrar.class) 来扩展包扫描路径，将其他包下的组件导入到主应用类所在的包下。
...
public @interface AutoConfigurationPackage {
  ...
  static class Registrar implements ImportBeanDefinitionRegistrar, DeterminableImports {
        public void registerBeanDefinitions(AnnotationMetadata metadata, BeanDefinitionRegistry registry) {
            // (String[])(new PackageImports(metadata)).getPackageNames().toArray(new String[0]) 就是main方法所在的包路径
...
***SpringBoot默认会在底层配好所有的组件，但是如果用户自己配置了以用户的优先。如果自己配置了就不会采用默认的***
...
**xxxxxAutoConfiguration ---> 按需配置组件 ---> xxxxProperties里面拿值  (绑定)----> application.properties**
...
<font color='red' size=6>只要静态资源放在类路径下： called `/static` (or `/public` or `/resources` or `/META-INF/resources`</font>

<font color='red'>访问 ： 当前项目根路径/ + 静态资源名</font>
...
spring:
#  mvc:
#    static-path-pattern: /res/**   这个会导致welcome 页功能失效
  resources:
    static-locations: [classpath:/haha/]
```
...
![image-20260313151301953](./image-20260313151301953.png)
	...
	public static class EnableWebMvcConfiguration extends DelegatingWebMvcConfiguration implements ResourceLoaderAware {
		...
		protected void addResourceHandlers(ResourceHandlerRegistry registry) {
			 if (!this.resourceProperties.isAddMappings()) {
				...
				// 静态资源路径规则
                 // getStaticPathPattern 来自于WebMvcProperties prefix = "spring.mvc"
                String staticPathPattern = this.mvcProperties.getStaticPathPattern();
                if (!registry.hasMappingForPattern(staticPathPattern)) {
                    // staticLocations = CLASSPATH_RESOURCE_LOCATIONS; 如果没有设置就采用默认的配置
...
public class WebMvcAutoConfiguration {
	...
	public static class EnableWebMvcConfiguration extends DelegatingWebMvcConfiguration implements ResourceLoaderAware {
		...
		public WelcomePageHandlerMapping welcomePageHandlerMapping(ApplicationContext applicationContext,
            ...
            // new WelcomePageHandlerMapping 构造方法
...
WelcomePageHandlerMapping(TemplateAvailabilityProviders templateAvailabilityProviders,
    ...
    // welcomePage:res/index.html  staticPathPattern:/res/** 对应的是yml中配置的  static-path-pattern: /res/**
...
```text
REST风格的主要特点
资源：在REST中，任何可以被命名的信息都可以被视为资源。资源可以是一个文档、一张图片、一个视频文件等。
统一接口：REST风格使用一组标准的HTTP方法来操作资源。这些方法包括：
GET：获取资源。例如，获取用户的详细信息。
POST：创建新的资源。例如，创建一个新的用户。
PUT：更新或替换资源。例如，更新用户的详细信息。
DELETE：删除资源。例如，删除一个用户。
PATCH：部分更新资源。例如，只更新用户的某个字段。
无状态：每个请求都是独立的，服务器不会存储客户端的状态。这意味着每次请求都必须包含所有必要的信息。(不需要session)
可缓存：响应可以被缓存，以提高性能和减少网络流量。
分层系统：客户端和服务器之间的通信可以通过中间层（如负载均衡器、代理服务器等）进行。
```
